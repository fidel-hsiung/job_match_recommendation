...
RSpec.describe Jobseeker do
  ...
  describe '#display_name' do
    it 'returns jobseeker\'s first name' do
      expect(jobseeker.display_name).to eq('Alice')
    end

    context 'when the name attribute is one word' do
      let(:name) { 'Test' }

      it 'returns name attribute' do
        expect(jobseeker.display_name).to eq(name)
      end
    end
  end
end
